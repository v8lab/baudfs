...
func (f *File) Open(ctx context.Context, req *fuse.OpenRequest, resp *fuse.OpenResponse) (handle fs.Handle, err error) {
	...
	sreader, err := f.super.ec.OpenForRead(ino)
	if err != nil {
		log.LogErrorf("Open: ino(%v) err(%v)", ino, err)
...
func (f *File) Read(ctx context.Context, req *fuse.ReadRequest, resp *fuse.ReadResponse) (err error) {
	...
	if f.sreader == nil {
		log.LogErrorf("Read unopen file: ino(%v)", f.inode.ino)
		return fuse.EPERM
	}

	start := time.Now()

	size, err := f.super.ec.Read(f.sreader, f.inode.ino, resp.Data[fuse.OutHeaderSize:], int(req.Offset), req.Size)
